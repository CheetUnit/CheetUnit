...
public class BaseInvokerGeneratorMojo extends AbstractMojo {
    ...
    private void createBaseInvokers(List<Class<?>> scopeAnnotatedClasses) throws MojoExecutionException {
        try {
            BaseInvokerFileCreator.createBaseInvokers(project.getBuild().getDirectory() + File.separator + "generated-test-sources", scopeAnnotatedClasses);
        } catch (IOException e) {
            e.printStackTrace();
